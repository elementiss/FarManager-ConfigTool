...
begin
  ...
  Writeln(f, 'Сравнение panelmodes: ', ExtractFileName(FileBase), ' → ', ExtractFileName(FileNew));
  Writeln(f, '---------------------------------------------------');
  ...
  if Assigned(BaseRoot) or Assigned(NewRoot) then begin // есть кастомные хотя бы в одном
    WriteLn(f, '=== CustomModes ==='#13#10);
